    ...
    for xtab in df.xtab_val.unique():
        ...
        for week in range(week_range[0], week_range[1]+1):
            if week in grouped_dict.index:
                ...
                values[week-1]['dateRange'] = dates[dates.week == week].dates.values[0]
            else:
...
def get_dates(engine, connection, metadata):    
    collection_dates = db.Table('collection_dates', metadata, autoload=True, autoload_with=engine)
    query = db.select([collection_dates]).order_by(db.desc(collection_dates.columns.week))
    # query
    result_proxy = connection.execute(query)
    result_keys = result_proxy.keys()
    result_data = result_proxy.fetchall()
    ...
    df = pd.DataFrame(result_data)
    df.columns = list(result_keys)
    df['dates'] = df.apply(lambda x: f"{datetime.strftime(x['start_date'], '%Y-%m-%d')} to {datetime.strftime(x['end_date'], '%Y-%m-%d')}", axis=1)
    df['date'] = df['end_date'].apply(lambda x:  datetime.strftime(x, '%Y-%m-%d'))
...
def get_xtab_labels():
    ...
    numeric_xtab = numeric_xtab.query("variable == 'TBIRTH_YEAR'")[['variable','bin', 'label']]
    numeric_xtab.columns = ['xtab_var', 'xtab_val', 'xtab_label']

    topline_xtab = pd.DataFrame([{ 'xtab_var': "TOPLINE", 'xtab_val': 1, 'xtab_label': "Summary" }])

    msa_xtabs = get_sheet("county_metro_state")[["cbsa_fips","cbsa_title"]].fillna('')
    msa_xtabs.columns = ['xtab_val','xtab_label']
    msa_xtabs['xtab_var'] = "EST_MSA"

    text_xtab = get_sheet("response_mapping").fillna('')
    text_xtab = text_xtab[text_xtab.variable.isin(xtab_labels.query_value)][["variable", "value", "label"]]
    text_xtab.columns = ["xtab_var", "xtab_val", "xtab_label"]
    
    combined_xtabs = pd.concat([numeric_xtab, topline_xtab, msa_xtabs, text_xtab])
...
def get_question_order(engine, connection, metadata): 
    ...
    most_recent = df.sort_values("week", ascending=False).groupby("q_var").first()
...
def get_questions(order_df, MIN_WEEK_FILTER):
    columns = ['variable_recode_final', 'variable_group_recode', 'variable_group', 'question_clean', 'exclude', 'topic_area', 'subtopic_area', 'drop_question', 'question_type']
    questions = get_sheet("question_mapping")[columns].fillna('')

    questions = questions.merge(order_df, left_on="variable_recode_final", right_on="q_var", how="left")
    questions.sort_values(by=["count_of_weeks", "most_recent_week"], ascending=[False, False], inplace=True)
    questions = questions[questions.count_of_weeks > MIN_WEEK_FILTER]
    questions.drop(
        columns=["q_var", "count_of_weeks", "most_recent_week", "variable_recode_final"],
        inplace=True
    )

    questions = questions.fillna('')
    questions['variable'] = questions.apply(lambda x: reconcile(x['variable_group_recode'], x['variable_group']), axis=1)
    ...
    questions['isMultiQuestion'] = questions['question_type'].apply(lambda x: x != "Select all")
    questions = questions[(questions['exclude'] != 1)&(questions['drop_question'] != 1)]
    questions.drop(
        columns=["exclude", "drop_question", "variable_group_recode", "variable_group"],
        inplace=True
    )
...
def get_question_groupings():
    columns = ["variable_recode_final", 'variable_group_recode', 'variable_group', 'question_type']
    questions = get_sheet("question_mapping")[columns].fillna('')

    questions['kind'] = questions['question_type'].apply(lambda x: handle_question_kind(x))
    questions['variable_group'] = questions.apply(lambda x: reconcile(x['variable_group_recode'], x['variable_group']), axis=1)
    questions = questions[["variable_recode_final", "variable_group", "kind"]]
    questions['variables'] = questions.apply(lambda x: json.dumps(list(questions[questions.variable_group == x.variable_group].variable_recode_final)), axis=1)
...
def get_label_groupings():
    ...
    response_mapping = get_sheet("response_mapping")[["variable", "variable_recode", "label", "label_recode", "value", "value_recode"]].fillna('')
    response_mapping["label"] = response_mapping.apply(lambda x: reconcile(x['label_recode'], x['label']), axis=1)
    response_mapping["variable"] = response_mapping.apply(lambda x: reconcile(x['variable_recode'], x['variable']), axis=1)
    response_mapping["value"] = response_mapping.apply(lambda x: reconcile(x['value_recode'], x['value']), axis=1)
    response_mapping = response_mapping[["variable", "value", "label"]]

    numeric_mapping = get_sheet("numeric_mapping")[["variable", "bin", "label"]].fillna('')
    numeric_mapping.columns = ["variable", "value", "label"]

    combined_mappings = pd.concat([response_mapping, numeric_mapping])

    merged_mappings = combined_mappings.merge(questions, how="left", left_on="variable", right_on="variable_recode_final")
    merged_mappings = merged_mappings[(merged_mappings.value != -99)&(merged_mappings.value != -88)]

    combined_labels = {}

    for variable_group in merged_mappings.variable_group.unique():
        temp_obj = {}
        sub_labels = merged_mappings[merged_mappings.variable_group == variable_group][["value", "label"]]
